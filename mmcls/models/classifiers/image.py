...
class ImageClassifier(BaseClassifier):
    ...
    def meta_extract_feat(self, img):
        """Directly extract features from the backbone + neck."""
        x = self.backbone(img)
        if self.return_tuple:
            if not isinstance(x, tuple):
    ...
    def forward_train(self, img, gt_label, meta=False, **kwargs):
        ...
        losses = dict()
        try:
            loss, cls_score = self.head.forward_train(x, gt_label)
    ...
    def meta_extract_feat2(self, img, metanetdict):
        """Directly extract features from the backbone + neck."""
        x = self.backbone.meta_forward(img, metanetdict)
        if self.return_tuple:
            if not isinstance(x, tuple):
                x = (x, )
                warnings.warn(
    ...
    def meta_feature_forward_train(self, img, gt_label, metanetdict, **kwargs):
        """Forward computation during training.
        ...
        x = self.meta_extract_feat2(img, metanetdict)
    ...
    def meta_forward_train(self, img, gt_label, **kwargs):
        """Forward computation during training.
        ...
        x = self.meta_extract_feat(img)

        # feats = [item.detach() for item in x]
        # feats_neck = [item.detach() for item in x_neck]
        # metas_input = {'feats': feats, 'feats_neck': feats_neck}
        try:
            cls_score = self.head.meta_forward(x)
        except TypeError as e:
            if 'not tuple' in str(e) and self.return_tuple:
                return TypeError(
                    'Seems the head cannot handle tuple input. We have '
